if(state >= 2)
{
    ...
    switch name {
        ...
        case 8:
            ...
            break;
        case 12:
            if pauseState == 3 draw_text_color(__view_get( e__VW.XView, 0 )+213, __view_get( e__VW.YView, 0 )+228, string_hash_to_newline(startText),c_white,c_white,c_yellow,c_yellow,1); start=true; break;
    }
    draw_set_halign(fa_center);
    draw_set_color(c_white);
    if(skipping > 0)
    ...
    if(allow_skipping)
    {
        if(name < 9 && is_fallback_touchscreen)
        {
           draw_sprite(sprTouchscreenFastForward, 0, __view_get( e__VW.XView, 0 ) + __view_get( e__VW.WView, 0 ) - 28, __view_get( e__VW.YView, 0 ) + 4);
        }
    }
}
if(state <= 2 || state == 4)
