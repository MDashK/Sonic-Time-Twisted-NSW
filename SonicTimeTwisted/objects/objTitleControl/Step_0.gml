...
switch(state)
{
    ...
    case 1:
        ...
        // on title screen, the alarm for deleting options file on touchscreens is disabled
        alarm[1] = -1;
        break;
    case 2:
        textAlpha -= 0.033;
...
if(menu.state == 0)
{
    ...
    if(is_fallback_touchscreen)
    {
        ...
        // erase/reset options by holding three fingers on screen at once while the names scroll
        if(name < 9)
        {
            var devicesPressed = 0;
            for(var i = 0; i < 4; i++)
            {
