/// @description  menu_fn_calculate_width_add(action, is_length_provided, labels...)
/// @param action
/// @param  is_length_provided
/// @param  labels...
...
if(argument_count > 2)
{
    ...
    calculated_width = string_width(string_hash_to_newline(label+" "));
    if(argument[1])
    {
        // adds a fixed length
        calculated_width += string_width(string_hash_to_newline(" ")) * argument[2];
    }
    else
    {
        ...
        {
            value_max_width = max(value_max_width, string_width(string_hash_to_newline(argument[i])));
        }
